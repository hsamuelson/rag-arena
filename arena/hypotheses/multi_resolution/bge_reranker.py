"""BGE-v2-m3 reranker on deep pool 50.

BAAI/bge-reranker-v2-m3 is a 568M-param cross-encoder trained on
multilingual data. Tests whether a much larger reranker improves
retrieval quality on the same candidate pool.
"""
...
class BGERerankerHypothesis(Hypothesis):
    """Retrieve top-50 candidates, BGE-v2-m3 rerank to top-10."""

    def __init__(self, model_name="BAAI/bge-reranker-v2-m3", final_k=10):
    ...
    def description(self):
        return f"Retrieve top-{self._pool_size}, BGE-v2-m3 (568M) rerank to top-{self._final_k}"
    ...
    def apply(self, query, results, embeddings, query_embedding):
        ...
        lines = [f"Retrieved context (deep pool {len(results)} → {len(top)}, BGE-v2-m3 reranked):"]
