#!/usr/bin/env python3
"""Re-run Late interaction and LI+MRAM with fixed model-specific prefixes."""
...
def main():
    ...
    print("=" * 70)
    print("LATE INTERACTION FIX — Re-running with correct model prefixes")
    print("=" * 70)
    ...
    # Build strategies — use snowflake for MaxSim too
    ...
    for name, hyp in strategies:
        print(f"\n{'=' * 60}")
        print(f"  Running: {name} (FIXED prefixes)")
        print(f"{'=' * 60}")
    ...
    print(f"\n{'#' * 70}")
    print("  COMPOSABILITY ANALYSIS (with fixed Late Interaction)")
    print(f"{'#' * 70}\n")

    ORIGINAL = {
        "Flat baseline": 0.2656, "CE L-12 (top-50)": 0.3720,
        "BGE-v2-m3 (top-50)": 0.3941, "MRAM-v1.5": 0.3789,
        "MRAM + BGE": 0.4056, "Late interaction": 0.3976,
    ...
    summary = {
        ...
        "note": "LI and LI+MRAM re-run with fixed model-specific prefixes",
