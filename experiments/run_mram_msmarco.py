#!/usr/bin/env python3
"""MRAM on MS MARCO: Scale test at 100K passages.

MS MARCO is the standard large-scale passage retrieval benchmark.
Full corpus is 8.8M passages — we sample 100K (including all gold passages
for our selected queries) to test whether sentence-level retrieval
still helps at 2x the scale of FiQA (57K docs).

Compares:
  1. CE baseline (hybrid BM25+dense, CE L-6, top-10)
  2. MRAM-v1.5 (sentence+passage, CE L-12)

Entity association (v2) is excluded — proven to have no effect on FiQA
and HotpotQA (see mram-experiment-results.md).

Published MS MARCO baselines (nDCG@10):
  - BM25:       0.228
  - DPR:        0.177
  - ANCE:       0.330
  - ColBERTv2:  0.397
...
from arena.benchmarks.ms_marco import MSMarcoBenchmark, MSMARCO_BASELINES
...
def main():
    ...
    print("=" * 70)
    print(f"MRAM on MS MARCO: Scale Test ({max_corpus // 1000}K passages)")
    print("=" * 70)
    ...
    samples = benchmark.samples()[:max_queries]
    ...
    # Hypotheses — CE baseline and MRAM-v1.5 only (entity layer excluded)
    ...
    # ─── Final Results ───
    print(f"\n{'#' * 70}")
    print("MRAM MS MARCO: RESULTS vs PUBLISHED BASELINES")
    print(f"{'#' * 70}")

    print(f"\n  Published baselines (MS MARCO nDCG@10):")
    for name, score in MSMARCO_BASELINES.items():
        print(f"    {name:20s}: {score:.4f}")

    print(f"\n  Our results ({len(corpus)} docs, {len(samples)} queries):")
    for name, data in results_data.items():
        ndcg = data["mean_ndcg"]
        vs_bm25 = (ndcg - MSMARCO_BASELINES["BM25"]) / MSMARCO_BASELINES["BM25"] * 100
        print(f"    {name:20s}: {ndcg:.4f}  ({vs_bm25:+.1f}% vs BM25)")

    # Statistical test: MRAM-v1.5 vs CE baseline
    ce_data = results_data.get("CE baseline")
    mram_data = results_data.get("MRAM-v1.5")
    if ce_data and mram_data:
        diff = (mram_data["mean_ndcg"] - ce_data["mean_ndcg"]) / ce_data["mean_ndcg"] * 100
        if mram_data["mean_ndcg"] >= ce_data["mean_ndcg"]:
            p_val = paired_permutation_test(mram_data["ndcgs"], ce_data["ndcgs"])
        else:
            p_val = paired_permutation_test(ce_data["ndcgs"], mram_data["ndcgs"])
        ci = bootstrap_ci(mram_data["ndcgs"])
        sig = "***" if p_val < 0.001 else "**" if p_val < 0.01 else "*" if p_val < 0.05 else "ns"
        print(f"\n  MRAM-v1.5 vs CE baseline: {diff:+.2f}% (p={p_val:.4f} {sig})")
        print(f"  MRAM-v1.5 95% CI: [{ci[0]:.4f}, {ci[1]:.4f}]")
    ...
    with open(output_dir / "mram_msmarco_summary.json", "w") as f:
        json.dump(summary, f, indent=2)

    print(f"\nSaved to {output_dir}/mram_msmarco_*.json")
